package com.tkzou.middleware.transaction.mqretry.v1.util;
...
public class KafkaUtil {
     ...
     * @return
     */
    public static KafkaConsumer<String, String> getConsumerInstance(String kafkaServerIp,
                                                                    String groupId,
                                                                    List<String> topics,
        ...
        if (isAutoCommit) {
            ...
            // 即在消费消息之前，先拿到offset值（但只拿一次，之后就不会再去zookeeper或kafka
            // 本地拿了，而是会在内存中获取，即该值也会在内存中存储并维护一份），开始从该值对应的消息消费起，
            ...
            // 1）若消息消费成功，但提交该消息对应的offset失败，则下次消费时还会从原来的offset对应的消息消费，也易知，会发生消息的重复消费
            ...
            // 易知如果想完成Consumer端的精准一次性消费，那么需要Kafka消费端将消费过程和提交offset过程做原子绑定，此时我们需要将Kafka的offset
            // 保存到支持事务的自定义介质（比如MySQL）中，demo在CustomSaveOffset中。
