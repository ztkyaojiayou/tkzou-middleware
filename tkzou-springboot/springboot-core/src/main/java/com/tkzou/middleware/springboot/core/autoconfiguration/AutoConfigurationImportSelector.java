...
public class AutoConfigurationImportSelector implements ImportSelector {
    ...
    public String[] selectImports(AnnotationMetadata importingClassMetadata) {
        // 收集自动配置类
        // 通过java的spi机制将目标java文件转为clazz字节码对象，其实也可以使用spring的spi机制，相对更集成，但本质是一样的！
        // 此时需要将这些要注入的bean先放在resources的META-INF.services目录下，否则扫描不到，就无法将它们加载为clazz对象，
        // 那么之后就无法再通过反射机制为其生成对象/bean！！！
